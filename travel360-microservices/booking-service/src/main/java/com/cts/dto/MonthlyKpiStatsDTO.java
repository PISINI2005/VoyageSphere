package com.cts.dto;

import lombok.AllArgsConstructor;
import lombok.Data;
import lombok.NoArgsConstructor;

/**
 * Money side of one month's KPI, sourced from the Payment ledger (windowed on paymentDate).
 * Counts are carried separately in {@link MonthlyKpiCountsDTO} and merged in the service.
 */
@Data
@AllArgsConstructor
@NoArgsConstructor
public class MonthlyKpiStatsDTO {
    private Integer month;
    private Double totalRevenue;        // net realized: SUM(SUCCESS) - SUM(REFUNDED)
    private Double flightRevenue;
    private Double hotelRevenue;
    private Double transportRevenue;
    private Double packageRevenue;
    private Double refundedAmount;      // actual refunds paid out: SUM(REFUNDED)
}
