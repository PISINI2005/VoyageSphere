...
public class TransportController {
    ...
    @GetMapping
    public ResponseEntity<Page<TransportResponseDTO>> getTransports(
        ...
        Page<TransportResponseDTO> result;
    ...
    @GetMapping("/{id}")
    public ResponseEntity<TransportResponseDTO> getById(
            @PathVariable Long id,
            @RequestParam(required = false)
            @DateTimeFormat(iso = DateTimeFormat.ISO.DATE)
            LocalDate date) {

        TransportResponseDTO transport = service.getTransportById(id, date);
        return ResponseEntity.ok(transport);
    }

    @Operation(summary = "Get dynamic price calendar for a specific transport class")
    @GetMapping("/{id}/calendar")
    public ResponseEntity<List<PriceDateDTO>> getPriceCalendar(
            @PathVariable Long id,
            @RequestParam String transportClass,
            @RequestParam(required = false) @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDate startDate,
            @RequestParam(required = false) @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDate endDate) {

        log.info("Fetching price calendar for transport ID: {}, transportClass: {}, range: {} to {}", id, transportClass, startDate, endDate);

        LocalDate start = (startDate == null) ? LocalDate.now() : startDate;
        LocalDate end = (endDate == null) ? LocalDate.now().plusDays(30) : endDate;

        List<PriceDateDTO> calendar = service.getPriceCalendar(id, transportClass, start, end);
