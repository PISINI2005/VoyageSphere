...
public class InvoiceController {
    ...
    @Operation(summary = "Get invoices for a specific user, or the current user if omitted")
    @GetMapping("/me")
    ...
    public ResponseEntity<Page<InvoiceResponseDTO>> getInvoices(
        ...
        return ResponseEntity.ok(
                service.getInvoicesByUser(userId, page, size)
        );
    }

    @Operation(summary = "Get invoices for a specific booking")
    @GetMapping("/booking/{bookingId}")
    @PreAuthorize("hasAnyRole('CUSTOMER','FINANCE_OFFICER','TRAVEL_AGENT','ADMIN')")
