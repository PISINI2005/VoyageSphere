...
public class BookingServiceImpl implements BookingService {
	...
	@Override
	public Page<BookingResponseDTO> getBookings(Long userId, Pageable pageable) {

	    Long targetUserId =
	            (userId != null) ? userId : authUser.current().getUserId();

	    authUser.assertCanActAs(targetUserId);

	    log.info("Fetching bookings for userId: {}", targetUserId);

	    Page<Booking> bookingPage =
	            bookingRepo.findByUserUserId(targetUserId, pageable);

	    log.info("Found {} bookings for userId: {}",
	...
	public Page<BookingResponseDTO> getAllBookings(Pageable pageable) {
	    ...
	    log.info("Fetching bookings. Page: {}, Size: {}",
	            pageable.getPageNumber(),
	            pageable.getPageSize());

	    Page<Booking> bookingPage = bookingRepo.findAll(pageable);

	    log.info("Bookings fetched: {}", bookingPage.getNumberOfElements());
	...
	@Override
	@Transactional
	public PassengerCancelResponseDTO cancelPassenger(Long bookingId, Long passengerId) {
		...
		if (activePassengers <= 1) {
			...
			cancelDto.setUserId(booking.getUser().getUserId());
		...
		notificationService.sendNotification(booking.getUser(), "Passenger " + passenger.getProfile().getPassengerName()
	...
	private void recordRefund(Booking booking, double refundAmount, PaymentStatus invoiceStatus, LocalDateTime now) {
		...
		Payment refundPayment = Payment.builder().invoice(paidInvoice).amount(refundAmount)
				.status(PaymentStatus.REFUNDED).paymentDate(now).paymentMethod("UPI").transactionId("TXN-REF" + java.util.UUID.randomUUID().toString().substring(0, 8).toUpperCase()).build();
